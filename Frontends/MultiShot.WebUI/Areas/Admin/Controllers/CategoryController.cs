...
namespace MultiShot.WebUI.Areas.Admin.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var client = _httpClientFactory.CreateClient();  // cagır clientı
            var responseMessage = await client.GetAsync("https://localhost:7070/api/Categories"); // istek at
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync(); // cevabı al
                var values = JsonConvert.DeserializeObject<List<ResultCategoryDto>>(jsonData); // jsonı deserialize et
                return View(values);
            }
            return View();
        }
