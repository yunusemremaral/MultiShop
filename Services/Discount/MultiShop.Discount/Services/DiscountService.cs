...
namespace MultiShop.Discount.Services
{
    public class DiscountService : IDiscountService
    {
        ...
        public async Task DeleteDiscountCouponAsync(int id)
        {
            ...
            using (var connection = _context.CreateConnection())
            {
                await connection.ExecuteAsync(query, parameters); // silme güncelleme eklemede execute
            }
        }
        ...
        public async Task<List<ResultDiscountCouponDto>> GetAllDiscountCouponAsync()
        {
            ...
            using (var connection = _context.CreateConnection())
            {
                var values = await connection.QueryAsync<ResultDiscountCouponDto>(query); //query async getirme işlemlerinde 
        ...
        public async Task<GetByIdDiscountCouponDto> GetByIdDiscountCouponAsync(int id)
        {
            ...
            using (var connection = _context.CreateConnection())
            {
                var values = await connection.QueryFirstOrDefaultAsync<GetByIdDiscountCouponDto>(query, parameters); // 1 veri getirirken kullanılır .
                return values;
            }
